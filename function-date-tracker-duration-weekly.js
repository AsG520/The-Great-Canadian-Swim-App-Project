...
function getDateFromWeek(weekValue) {
    ...
    firstSunday.setDate(
        januaryFirst.getDate() - januaryFirst.getDay()
    );
...
averageUnitSelection.addEventListener("change", function () {
    ...
    averageDurationDisplay.value =
        displayedAverageDuration.toFixed(2);
});
...
function loadWeek(date) {
    ...
    // Store duration for each day
    let dailyDurations = [0, 0, 0, 0, 0, 0, 0];
    ...
    averageDuration = totalDuration / 7;
    ...
    // Convert daily durations from hours to minutes
    let graphDurations = [];

    for (let i = 0; i < dailyDurations.length; i++) {
        graphDurations.push(dailyDurations[i] * 60);
    }

    // Draw the graph in minutes
